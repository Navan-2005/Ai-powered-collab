...
const Project = () => {
    ...
    useEffect(() => {
        ...
        // In your Project.jsx file, modify the receiveMessage handler to add better error handling
receiveMessage('project-message', data => {
    ...
    if (data.sender._id == 'ai') {
        try {
            ...
            // Handle invalid JSON by displaying the raw message
            data.message = JSON.stringify({ text: "Error parsing AI message: " + data.message });
        }
        ...
        <main className='h-screen w-screen flex'>
            <section className="left relative flex flex-col h-screen min-w-96 bg-slate-300">
                ...
                <div className={`sidePanel w-full h-full flex flex-col gap-2 bg-slate-50 absolute transition-all ${isSidePanelOpen ? 'translate-x-0' : '-translate-x-full'} top-0`}>
                    ...
                    // Check if user and user._id exist before using them
                    return (
                        <div className="user cursor-pointer hover:bg-slate-200 p-2 flex gap-2 items-center">
                            <div className='aspect-square rounded-full w-fit h-fit flex items-center justify-center p-5 text-white bg-slate-600'>
                                <i className="ri-user-fill absolute"></i>
                            </div>
                            <h1 className='font-semibold text-lg'>{user && user.email ? user.email : 'Unknown User'}</h1>
                        </div>
                    )
                })}
                    </div>
